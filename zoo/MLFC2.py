...
class Conv2d_batchnorm(torch.nn.Module):
    ...
    def __init__(
        ...
        self.goupnorm = nn.GroupNorm(num_groups = 32 ,num_channels= num_out_filters)
...
class MLFC(nn.Module):
    ...
    def __init__(self, in_filters1, in_filters2, in_filters3,in_filters4, lenn=1):
        ...
        for i in range(lenn):
            ...
            # self.bns1.append(torch.nn.GroupNorm(num_groups=32, num_channels=in_filters1))
            # self.bns_mrg1.append(torch.nn.GroupNorm(num_groups=32, num_channels=in_filters1))
            ...
            # self.bns2.append(torch.nn.GroupNorm(num_groups=32, num_channels=in_filters2))
            # self.bns_mrg2.append(torch.nn.GroupNorm(num_groups=32, num_channels=in_filters2))
            ...
            # self.bns3.append(torch.nn.GroupNorm(num_groups=32, num_channels=in_filters3))
            # self.bns_mrg3.append(torch.nn.GroupNorm(num_groups=32, num_channels=in_filters3))
    ...
    def forward(self, x1, x2, x3,x4):
        ...
        for i in range(len(self.cnv_blks1)):
            ...
            x_c1 = self.act(
                self.bns_mrg1[i](
                    self.cnv_mrg1[i](
                        ...
                        torch.cat([x_c1, x1], dim=2).reshape(batch_size, 2 * self.in_filters1, h1, w1)
                    )
                    + x1
                )
            )
            x_c2 = self.act(
                self.bns_mrg2[i](
                    self.cnv_mrg2[i](
                        ...
                        torch.cat([x_c2, x2], dim=2).reshape(batch_size, 2 * self.in_filters2, h2, w2)
                    )
                    + x2
                )
            )
            x_c3 = self.act(
                self.bns_mrg3[i](
                    self.cnv_mrg3[i](
                        ...
                        torch.cat([x_c3, x3], dim=2).reshape(batch_size, 2 * self.in_filters3, h3, w3)
                    )
                    + x3
                )
            )
            x_c4 = self.act(
                self.bns_mrg4[i](
                    self.cnv_mrg4[i](
                        torch.cat([x_c4, x4], dim=2).reshape(batch_size, 2 * self.in_filters4, h4, w4)
                    )
                    + x4
                )
            )
