...
import timeit
import cv2
from zoo.models_MLFC2 import SeResNext50_Unet_Loc
# from zoo.models_ECAconv import SeResNext50_Unet_Loc
# from zoo.models_cbam import SeResNext50_Unet_Loc
# from zoo.models_cbamconv import SeResNext50_Unet_Loc
...
class TrainData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # msk0 = cv2.imread(fn.replace('/sat/', '/masks/'), 0)
...
def train_epoch(current_epoch, seg_loss, model, optimizer, scheduler, train_data_loader):
    ...
    for i, sample in enumerate(iterator):
        ...
        # flops, params = get_model_complexity_info(model, (imgs), as_strings=True, print_per_layer_stat=False)
        # print('flops: ', flops, 'params: ', params)

        iterator.set_description(
            "epoch: {}; lr {:.7f}; Loss {loss.val:.4f} ({loss.avg:.4f}); Dice {dice.val:.4f} ({dice.avg:.4f}); Nancount {nancount}".format(
                current_epoch, scheduler.get_lr()[-1], loss=losses, dice=dices,nancount=nanitem))
    ...
    f.write('epoch\b{0}\b'
            'lr\b{1:.9f}\b'
            'loss\b{loss.val:.4f}\b{loss.avg:.4f}\b'
            'Dice\b{dice.val:.5f}\b{dice.avg:.5f}\b'.format(
        current_epoch, scheduler.get_lr()[-1], loss=losses, dice=dices))
    f.close()
...
if __name__ == '__main__':
    ...
    snapshot_name = 'daolu_{}'.format(seed)
