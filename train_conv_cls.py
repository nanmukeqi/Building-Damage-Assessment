...
from zoo.models_cbamconv import SeResNext50_Unet_Double
...
class ValData(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        msk_loc = cv2.imread(path.join(loc_folder, f), cv2.IMREAD_UNCHANGED) > (0.3 * 255)
        msk0 = cv2.imread(fn.replace('/images/', '/masks/'), cv2.IMREAD_UNCHANGED)
        lbl_msk1 = cv2.imread(fn.replace('/images/', '/masks/').replace('_pre_disaster', '_post_disaster'),
                              cv2.IMREAD_UNCHANGED)
        msk1 = np.zeros_like(lbl_msk1)
...
if __name__ == '__main__':
    ...
    snapshot_name = 'conv_cls_{}_0'.format(seed)
    ...
    snap_to_load = 'conv_cbam_27_2_0_best'
